...
class HistoricalVolatilitySample():
    def on_start(self):
        self.volumeInUnits = api.Symbol.QuantityToVolumeInUnits(api.VolumeInLots)
        self.historicalVolatility = api.Indicators.HistoricalVolatility(api.Bars.ClosePrices, api.Periods, api.BarHistory);
        self.simpleMovingAverage = api.Indicators.SimpleMovingAverage(api.Bars.ClosePrices, 9);

    def on_bar_closed(self):
        if self.historicalVolatility.Result.Last(0) < Functions.Maximum(self.historicalVolatility.Result, 14):
            return
