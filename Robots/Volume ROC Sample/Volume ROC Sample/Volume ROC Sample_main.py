...
class VolumeROCSample():
    def on_start(self):
        self.volumeInUnits = api.Symbol.QuantityToVolumeInUnits(api.VolumeInLots)
        self.volumeROC = api.Indicators.VolumeROC(api.Periods)
        self.volumeROCSimpleMovingAverage = api.Indicators.SimpleMovingAverage(self.volumeROC.Result, 14)
        self.priceSimpleMovingAverage = api.Indicators.SimpleMovingAverage(api.Bars.ClosePrices, 14)

    def on_bar_closed(self):
        if self.volumeROC.Result.Last(0) < self.volumeROCSimpleMovingAverage.Result.Last(0):
            return
